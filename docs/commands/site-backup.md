...
- HTTPS endpoints are mandatory unless `--allow-insecure` was used at storage setup, which persists `WPFY_BACKUP_S3_ALLOW_INSECURE=1`. Plaintext HTTP exposes backup contents and SigV4 credentials and fails closed unless that opt-out is set.
- S3 redirects that change scheme or host:port are refused before any signed request can reach the redirect target; this prevents HTTPS-to-HTTP downgrade even when `--allow-insecure` configured the original endpoint.
