...
## 2026-08-23: Panel account self-service keyed to session identity; basic-auth state must be honest about enforcement
- Decision: Open profile, password-change, TOTP, and session-revocation routes to every signed-in user including site managers, keyed to the authenticated principal with client-supplied usernames ignored. Password change requires the current secret (400 on mismatch), revokes sibling sessions, and keeps the acting one. TOTP enrollment becomes cancellable server-side (`DELETE /api/auth/totp/pending`). Basic-auth settings carry a server-derived `auth_state` (`enforced`/`staged`/`stale`/`unknown`/`off`) read from the router's own content, where `stale` names the case where a router enforces a different-or-orphaned credential and the old prompt is still live; disable restores the stored credential when a recognized router's rewrite fails, and refuses with 409 when the panel is exposed but no managed router is recognized. See ADR 0033 (amended).
- Reason: Account hygiene (rotate a leaked password, kill a lost device's session, enroll a second factor) is per-identity work that managers otherwise need admin help for, and keying to the session makes the grant safe without system scope. A cancel-less enrollment flow trapped cautious operators behind a disclosure TTL. Reporting a credential "enabled" while nothing enforces it -- or deleting it while an unmanaged router still prompts -- turns the panel into the source of the lie it exists to prevent.
- Alternatives considered: admin-only account routes (rejected: makes managers depend on admins for self-service basics); honoring body usernames (rejected: identity confusion and privilege confusion in one field); deleting the credential on unrecognized-router exposure (rejected: reports success while the prompt may survive); 401 for wrong current password (rejected: the shell signs out on any 401, destroying the form).
- Consequence: The manager allowlist grows by five actions (`auth.totp.cancel` joins the four rc6 routes); new auth routes still deny by default. Enabling TOTP invalidates all sessions via the credential fingerprint, disclosed in the UI. Disable on an unrecognized-router host stays refused until the operator resolves the router out-of-band; the error names the cause.
- Status: Accepted.

## 2026-08-26: IPv6 is enforced at the edge, not per site; every protection claim is gated on the running daemon
- Decision: wpfy manages `ipv6`, `ip6tables` and `fixed-cidr-v6` in `/etc/docker/daemon.json` (merged, never clobbered; unparseable JSON refused; backed up before the first write), and never restarts the daemon — the operator does that, told what it costs. All wpfy IPv6 subnets are `/64`s carved from one ULA prefix `fd4a:3b1c::/48`, with reserved indices 0 = docker0, 1 = the `wpfy` network, 2 = `wpfy-panel-edge`. Per-site networks stay IPv4-only. A live network lacking IPv6 is refused, never silently recreated; `wpfy stack ipv6-migrate --force` is the only sanctioned path and is disclosed as a full-stack maintenance window. `IPv6 protection: active` is claimed only when the *running* daemon reports IPv6 on, never from the file wpfy just wrote. `fc00::/7` and `::` join the never-ban set in the panel and in the WordPress bridge. See ADR 0036.
