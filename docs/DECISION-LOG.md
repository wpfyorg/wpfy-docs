...
## 2026-08-10: Panel exposure — canonical host, ACME prerequisites, trust boundary, atomic rollback
- Decision: Record the exposure contract as implemented: loopback default with SSH tunnelling recommended; opt-in `wpfy panel expose` gated on exact typed confirmation, named-user login, an enrolled TOTP factor, DNS/IP preflight (never skipped), and a valid non-local ACME contact on every resolver plus the `le-http` HTTP-01 resolver; one canonical HTTPS router with aliases getting SAN coverage and redirecting to the canonical host; HTTP-to-HTTPS redirect router pinned to `service: noop@internal`; forwarded identity believed only from a `wpfy-panel-edge` peer; atomic rollback via `wpfy panel expose --disable`. See ADR 0032.
- Reason: The release rehearsal (2026-08-09/10, staging host 155.94.241.76, canonical panel.wpfydev.top, ACME contact arnab@wpfy.org) proved TLS issuance and the auth boundary but failed the HTTP-to-HTTPS redirect (D1: Traefik v3 rejected a service-less router, 404 on port 80) and could not verify IPv6 (host lacks global IPv6). The hotfix adding `noop@internal` requires deployment; nothing is described as live until deployed.
- Alternatives considered: tunnel-only exposure (rejected: browser-only operators need a supported published path); aliases serving panel content directly (rejected: keeps one cross-origin surface); a service-less redirect router (rejected live); skipping the ACME contact check (rejected: registration needs a valid contact).
- Consequence: A valid ACME contact and the `le-http` resolver are documented exposure prerequisites; D1 and the FM proxy Host-header fix (D6) are stated as requiring deployment; IPv6 exposure is reported NOT VERIFIED; firewall policy remains the operator's responsibility.
- Status: Accepted.

## 2026-08-10: File Manager legacy API default flipped to disabled
- Decision: `WPFY_FM_LEGACY_API` now defaults to off (`0`); the legacy file-manager API returns HTTP 404 "file manager legacy api disabled" unless `WPFY_FM_LEGACY_API=1` is set explicitly. Setting `1` remains the deliberate rollback path. See ADR 0031 (amended).
- Reason: The F-FILES-PROTECT fix closed the legacy surface by default after ADR 0031's original text recorded the old on-by-default rollback shape; the release rehearsal verified the 404 live.
- Alternatives considered: keeping the legacy API on by default (rejected: the protected-file boundary would be reachable without an explicit opt-in).
- Consequence: Sites that relied on the legacy file API must set `WPFY_FM_LEGACY_API=1` explicitly; Quantum remains the primary surface and the D6 Host-header proxy fix requires deployment.
- Status: Accepted.

## 2026-08-10: Panel throttle key and Login Shield gate truth
- Decision: Record that ADR 0028 supersedes ADR 0022 for the panel throttle key (trusted-edge forwarded client instead of socket peer), and that the phase 7e gates B5/B6 assert the pre-amendment Login Shield design and remain red (16 passed / 2 failed) as documented stale probes, not security regressions. See ADR 0022 (superseded in part) and ADR 0023 (amended 2026-08-10).
- Reason: The release rehearsal re-verified Login Shield end to end (per-site ban on DOCKER-USER, Site-B untouched, SSH untouched, unban restores) while the two gates still encode the old access-log/iptables-multiport expectations, which the audit treats as a blocked release signal.
- Alternatives considered: rewriting the two gate expectations in this todo (rejected: Todo 16 is documentation only; test maintenance is tracked separately), or relabelling the failing gates as passing (rejected: no unsupported PASS claims).
- Consequence: Docs now state that B5/B6 are stale probes against the amended design, that live enforcement is proven, and that fixing the gates is tracked release-blocking test maintenance.
- Status: Accepted.
