...
Basic authentication state is authoritative in `security.json`, while the credential hash is kept at `<site>/nginx/htpasswd`, outside the served `app/` document root. New credentials use a fresh-salt Apache APR1 (`$apr1$`) hash. The shipped nginx accepts APR1, while Python 3.13 no longer has `crypt` and wpfy cannot add a runtime dependency for bcrypt or sha512crypt. APR1 is MD5-based and not a modern password KDF, but is a material improvement over unsalted `{SHA}` SHA-1 under this platform constraint. Existing `{SHA}` entries remain nginx-compatible; operators should rotate them after upgrading. The file is mounted read-only at `/etc/nginx/wpfy-htpasswd` and uses mode `0640` with the site's Unix uid. Because it is an individual Compose bind mount, password rotation and revocation update the existing inode in place with `O_NOFOLLOW`; atomic replacement would leave a running Nginx container reading the old credential inode. Restore reapplies mode `0640`, then the shared ownership pass reapplies the site's uid:gid before runtime restart. The generated health endpoint explicitly disables inherited basic auth so Docker's internal healthcheck continues to work. Passwords are generated or accepted through stdin/prompt, returned once when generated, and never persisted in state or events.
...
- Use bcrypt or sha512crypt: preferred cryptographically, but rejected for this release because Python 3.13 removed `crypt` and wpfy has no reachable stdlib hasher or permitted runtime dependency.
- Keep `{SHA}` for new credentials: rejected because it is an unsalted SHA-1 digest; legacy entries remain readable only for compatibility.
...
- APR1 salts and stretches new credentials, but its MD5-based construction is not equivalent to bcrypt. Existing `{SHA}` credentials keep working and should be rotated after upgrade. Restored htpasswd files return to `0640` and the site uid:gid before nginx restarts.
