...
- Use bcrypt: rejected because it requires a runtime dependency.
- Keep APR1 as the default: rejected because it is MD5-based and `openssl passwd -6 -stdin` is available on supported targets without exposing a password in argv.
- Keep `{SHA}` for new credentials: rejected because it is an unsalted SHA-1 digest.
...
- sha512crypt is the default credential scheme. APR1 is an explicit host-capability fallback recorded in the operation event. Restored htpasswd files return to `0640` and the site uid:gid before nginx restarts.
