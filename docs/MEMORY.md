...
- Full VPS beta validation pass completed 2026-06-04 on 155.94.241.76 (Ubuntu 24.04, *.wpfydev.top). All 13 phases clean. Real ACME cert issued, post-reboot persistence confirmed, HTTP hardening probed live. Accepted residual: php-fpm images run as root (USER www-data deferred — needs volume ownership strategy).
...
- `site restore` validates backup archive members before stopping runtime or extracting, and rejects path traversal, absolute paths, links, device files, and archives rooted at a different domain.
...
## Planned / Deferred
- Remediate remaining security audit findings: Traefik socket risk reduction, supply-chain verification for installer archives and WP-CLI downloads, explicit non-root users/read-only root filesystems where compatible, and live execution of the disposable-VPS validation flow.
